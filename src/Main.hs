...
main = do 
        ...
        when True $ do
          u <- f_subVec3 [1, 2, 3] [2, 3, 4]
          print "u"
          print u 

        when True $ do
          w <- f_mulScaleVec3 4 [1, 2, 3]
          print "w"
          print w 

        when True $ do
          k <- f_mulVec3Scale [1, 2, 3] 4
          print "k"
          print k 
        ...
        when True $ do
          ...
          {--
        when True $ do
          let vs = V.fromList ([0, 0, 0] :: [CDouble])
          v <- V.thaw vs
          VM.unsafeWith v $ \pt -> 
            c_sortx 3 pt
          out <- V.freeze v
...
--         let b = -3
--         let c = 2
--         t@(x0, x1, nv) <- f_gsl_poly_solve_quadratic a b c
--         print t
